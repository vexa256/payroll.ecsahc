    ...
    <table class="mytable table table-rounded table-bordered border gy-3 gs-3">
        <thead>
            <tr class="fw-bold text-gray-800 border-bottom border-gray-200">
                <td>{{ $data->id }}</td><td>{{ $data->ConversionID }}</td><td>{{ $data->USD_To_TSH_Rate }}</td><td>{{ $data->PayrollMonth }}</td><td>{{ $data->PayrollYear }}</td><td>{{ $data->created_at }}</td><td>{{ $data->updated_at }}</td>
                                ...
                                <ul class="dropdown-menu"
                                    ...
                                    <li>
                                        <a data-msg="You want to delete this record. This action is not reversible!!"
                                            data-route="{{ route('MassDelete', ['TableName' => 'currency_conversions', 'id' => $data->id]) }}"
    ...
    @foreach ($Roles as $up)
        ...
        <form action="{{ route('MassUpdate') }}" class="" method="POST">
            ...
            <div class="row">
                ...
                <input type="hidden" name="TableName" value="currency_conversions">

                {{ RunUpdateModalFinal($ModalID = $up->id, $Extra = '', $csrf = null, $Title = null, $RecordID = $up->id, $col = '4', $te = '12', $TableName = 'currency_conversions') }}
            </div>
