    ...
    <table class="mytable table table-rounded table-bordered border gy-3 gs-3">
        <thead>
            <tr class="fw-bold text-gray-800 border-bottom border-gray-200">
                <td>{{ $data->id }}</td><td>{{ $data->tokenable_type }}</td><td>{{ $data->tokenable_id }}</td><td>{{ $data->name }}</td><td>{{ $data->token }}</td><td>{{ $data->abilities }}</td><td>{{ $data->last_used_at }}</td><td>{{ $data->expires_at }}</td><td>{{ $data->created_at }}</td><td>{{ $data->updated_at }}</td>
                                ...
                                <ul class="dropdown-menu"
                                    ...
                                    <li>
                                        <a data-msg="You want to delete this record. This action is not reversible!!"
                                            data-route="{{ route('MassDelete', ['TableName' => 'personal_access_tokens', 'id' => $data->id]) }}"
    ...
    @foreach ($Roles as $up)
        ...
        <form action="{{ route('MassUpdate') }}" class="" method="POST">
            ...
            <div class="row">
                ...
                <input type="hidden" name="TableName" value="personal_access_tokens">

                {{ RunUpdateModalFinal($ModalID = $up->id, $Extra = '', $csrf = null, $Title = null, $RecordID = $up->id, $col = '4', $te = '12', $TableName = 'personal_access_tokens') }}
            </div>
